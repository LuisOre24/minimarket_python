...
from app.users.usersModel import UsersModel
...
class AuthController:
    ...
    def auth_login(self):
        ...
        if form_login.validate_on_submit():
            ...
            user = UsersModel.query.filter_by(user=username).first() # None
    ...
    def auth_register(self):
        ...
        if form_register.validate_on_submit():
            username = form_register.user.data
            password = form_register.password.data

            # Validamos que el usuario no exista
            user = UsersModel.query.filter_by(user=username).first()
            if user:
